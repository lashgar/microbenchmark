...
int main()
{
    int i, j, k; int nthread = NTHD, iteration = 2, chunk = 128 / sizeof(int); 
...
#define dummy1(index) __smc_select_0_dummy1(index, (blockIdx.x*512)-(1), ((blockIdx.x+1)*512)+(1), dummy1, __kernel_smc_var_data_dummy1, 512, 0, 1)
...

                    {
                        if (i < nthread) {
                            ...
                            __syncthreads();
                            __smc_write_0_dummy1(i, (blockIdx.x*512)-(1), ((blockIdx.x+1)*512)+(1), dummy1, __kernel_smc_var_data_dummy1, 512, 0, 1, addr1 + addr2 + 0);
                            __syncthreads();
                            __syncthreads();
                            tick = clock();
