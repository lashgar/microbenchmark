...
int main()
{
    int i, j, k; int nthread = NTHD, iteration = 7, chunk = 128 / sizeof(int); 
...
#define dummy1(index) __smc_select_0_dummy1(index, (blockIdx.x*512)-(1), ((blockIdx.x+1)*512)+(1), dummy1, __kernel_smc_var_data_dummy1, 512, 0, 1)
...

                    {
                        if (i < nthread) {
                            ...
                            unsigned long long int addr7 = (unsigned long long int)dummy2 + ((threadIdx.x) + 32 * (d2 - d1) + ((blockDim.x / DISP) * 7)) * chunk * sizeof(int);
                            __syncthreads();
                            __smc_write_0_dummy1(i, (blockIdx.x*512)-(1), ((blockIdx.x+1)*512)+(1), dummy1, __kernel_smc_var_data_dummy1, 512, 0, 1, addr1 + addr2 + addr3 + addr4 + addr5 + addr6 + addr7 + 0);
                            __syncthreads();
                            __syncthreads();
                            tick = clock();
                            ...
                            int reg7; asm ("ld.global.s32 %0, [%1+0];" : "=r" (reg7) : "l" (addr7));
                            reg1 = reg1 + reg2 + reg3 + reg4 + reg5 + reg6 + reg7 + 0;
                            __syncthreads();
                            tock = clock();
                            __syncthreads();
